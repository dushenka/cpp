...
short mainMenuChoise, settingsChoise, menuColorChoise, gridChoise = 1, firstTurnChoise = 1, gameModeChoise, playerXChoise, playerOChoise, turnCPU, fieldOverflowStatus = 0, xColor = 2, oColor = 1;
string color, backgroundColorChoise, foregroundColorChoise;
char gameField[5][5] = { { '#', '#', '#', '#', '#' }, {'#', '#', '#', '#', '#'}, {'#', '#', '#', '#', '#'}, {'#', '#', '#', '#', '#'} , {'#', '#', '#', '#', '#'} };
char winner = ' ';

string clrRed = "\033[31", clrGreen = "\033[32", clrYellow = "\033[33", clrBlue = "\033[34", clrPurple = "\033[35", clrCyan = "\033[36", clrRst = "\033[0m";
...
void xoColor()
{
	cout << " [c] Available colors for X and O: \n\n";
	cout << " [1] - Red\n";
	cout << " [2] - Green\n";
	cout << " [3] - Yellow\n";
	cout << " [4] - Blue\n";
...
void printGameField()
{
	...
	for (short y = 0; y < i; y++)
	{
		for (short x = 0; x < i; x++)
		{
			cout << " " << gameField[x][y];
			//
			// IFs colors if x14 (7 color x 2 marks)
			// cout << colorX + "X" + colorReset << endl;
			//
		}
		cout << endl;
	}
